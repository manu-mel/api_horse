...
type
  ...
  protected
    ...
    function DataSetToJSONObject(dataSet: TDataSet; AParamCase: TParamCase = pcLowerCase): TJSONObject;
    function DataSetToJSONArray(dataSet: TDataSet; AParamCase: TParamCase = pcLowerCase): TJSONArray;
    ...
    function AsJSONObject(AParamCase: TParamCase = pcLowerCase): TJSONObject;
    function AsJSONArray(AParamCase: TParamCase = pcLowerCase): TJSONArray;
...
function TDataSetConverter.AsJSONArray(AParamCase: TParamCase): TJSONArray;
begin
  Result := DataSetToJSONArray(GetDataSet, AParamCase);
end;

function TDataSetConverter.AsJSONObject(AParamCase: TParamCase): TJSONObject;
begin
  Result := DataSetToJSONObject(GetDataSet, AParamCase);
end;
...
function TDataSetConverter.DataSetToJSONArray(dataSet: TDataSet; AParamCase: TParamCase = pcLowerCase): TJSONArray;
var
  bookMark: TBookmark;
begin
  Result := nil;
  if Assigned(dataSet) and (not dataSet.IsEmpty) then
    try
      ...
      begin
        Result.AddElement(DataSetToJSONObject(dataSet, AParamCase));
        dataSet.Next;
      end;
    finally
...
function TDataSetConverter.DataSetToJSONObject(dataSet: TDataSet; AParamCase: TParamCase = pcLowerCase): TJSONObject;
var
  i: Integer;
  key: string;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if AParamCase = pcLowerCase then
          key := LowerCase(key);

        if AParamCase = pcUpperCase then
          key := UpperCase(key);
          ...
          TFieldType.ftLongWord, TFieldType.ftAutoInc:
            begin
              if not dataSet.Fields[i].IsNull then
                ...
                Result.AddPair(key, TJSONNumber.Create(0)); // EXCIA
//                Result.AddPair(key, TJSONNull.Create);
            end;
          TFieldType.ftLargeint:
            Result.AddPair(key, TJSONNumber.Create(dataSet.Fields[i].AsLargeInt));
          TFieldType.ftSingle, TFieldType.ftFloat, TFieldType.ftCurrency:
          ...
          ftString, ftWideString, ftMemo, ftWideMemo:
            begin
              if not dataSet.Fields[i].IsNull then
                ...
                Result.AddPair(key, TJSONString.Create(''));
            end;
          TFieldType.ftDate, TFieldType.ftDateTime:
            begin
              if not dataSet.Fields[i].IsNull then
                 Result.AddPair(key, TJSONString.Create(DateToISODate(dataSet.Fields[i].AsDateTime)))
//                Result.AddPair(key, TJSONString.Create(FormatDateTime('DD/MM/YYYY',dataSet.Fields[i].AsDateTime))) // EXCIA
              else
//                Result.AddPair(key, TJSONString.Create('')); // EXCIA
                Result.AddPair(key, TJSONNull.Create);
            end;
//          EXCIA
...
//                Result.AddPair(key, TJSONNull.Create);
//            end;
          TFieldType.ftTime, TFieldType.ftTimeStamp:
            begin
              if not dataSet.Fields[i].IsNull then
              begin
...
//          TFieldType.ftCurrency:     // EXCIA trataremos currency como float
//            begin
//              if not dataSet.Fields[i].IsNull then
...
//                Result.AddPair(key, TJSONNull.Create);
          ...
          TFieldType.ftDataSet:
            begin
              ...
              case dft of
                dfJSONObject:
                  Result.AddPair(key, DataSetToJSONObject(nestedDataSet,AParamCase));
                dfJSONArray:
                  Result.AddPair(key, DataSetToJSONArray(nestedDataSet,AParamCase));
              end;
            end;
...
begin
  ...
  begin
    ...
    begin
      if Assigned(json.Get(LowerCase(field.FieldName))) then   // EXCIA - adicionado o lowercase
        jv := json.Get(LowerCase(field.FieldName)).JsonValue
      else
        Continue;
      ...
      case field.DataType of
        ...
        TFieldType.ftInteger, TFieldType.ftSmallint, TFieldType.ftShortint, TFieldType.ftLongWord:
          begin
            ...
            begin
              if not TryStrToInt(jv.Value,iValue) then
                raise Exception.Create('Campo em formato inválido: '+LowerCase(field.FieldName));

              field.AsInteger := iValue;
            end;
//            EXCIA
...
//              field.AsLargeInt := StrToInt64Def(jv.Value, 0);
        ...
        TFieldType.ftCurrency:
          begin
            ...
            begin                 // tratativa p poder informar valores todo errado
              if not TryStrToCurr(StringReplace(jv.Value, '.', ',', []),cValue) then
                raise Exception.Create('Campo em formato inválido: '+LowerCase(field.FieldName));

              field.AsCurrency := cValue;
            end;
//            EXCIA
//              field.AsCurrency := StrToCurr(jv.Value);
        ...
        TFieldType.ftFloat, TFieldType.ftFMTBcd, TFieldType.ftBCD, TFieldType.ftSingle:
          begin
            ...
            begin                    // tratativa p poder informar valores todo errado
              if not TryStrToFloat(StringReplace(jv.Value, '.', ',', []),vValue) then
                raise Exception.Create('Campo em formato inválido: '+LowerCase(field.FieldName));

              field.AsFloat := vValue;
            end;
//            EXCIA
//              field.AsFloat := StrToFloatDef(jv.Value,0); // EXCIA StrToFloatDef
          end;
        ftString, ftWideString, ftMemo, ftWideMemo:
          begin
            if jv is TJSONNull then
        ...
        TFieldType.ftDate:
          begin
            if (jv is TJSONNull) or (jv.Value = '') then
              field.AsDateTime := 0
            else
//              field.AsDateTime := StrToDateDef(jv.Value,Date); // EXCIA
...
//              field.AsDateTime := StrToDateTimeDef(jv.Value,Date); // EXCIA
        ...
        TFieldType.ftGraphic, TFieldType.ftBlob, TFieldType.ftStream:
          begin
            ...
            begin
              ...
              try
                TBlobField(field).LoadFromStream(ss);
              finally
                ss.Free;
              end;
